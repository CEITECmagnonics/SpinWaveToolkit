...
CoFeBchar = SWT.DispersionCharacteristic(kxi = kxi, theta = np.pi/2, phi = np.deg2rad(90), d = 100e-9, boundaryCond = 4, dp = 8e6, Bext = 0.08, material = MagnetronCoFeB)
...
#NiFeCharBV = SWT.DispersionCharacteristic(kxi = kxi, theta = np.pi/2, phi = 0, d = 30e-9, boundaryCond = 1, Bext = 0.1, material = SWT.CoFeB)
#
#f00 = NiFeChar.GetDispersion(n=0)*1e-9/(2*np.pi) 
#f00BV = NiFeCharBV.GetDispersion(n=0)*1e-9/(2*np.pi) 
...
#
#plt.plot(kxi*1e-6, f00, kxi*1e-6, f11, kxi*1e-6, f00BV);
#plt.xlabel('kxi (rad/um)');
#plt.ylabel('Frequency (GHz)');
#plt.legend(['00', '11'])
