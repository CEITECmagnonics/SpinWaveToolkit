...
CoFeBchar = SWT.DispersionCharacteristic(kxi = kxi, theta = np.pi/2, phi = np.pi/2, d = 100e-9, boundaryCond = 4, dp = 1.7e7, Bext = 0.08, material = MagnetronCoFeB)
w00 = CoFeBchar.GetDispersion(n=0)*1e-9/(2*np.pi)
w22 = CoFeBchar.GetDispersion(n=3)*1e-9/(2*np.pi)
[wd00, wd22] =  [w*1e-9/(2*np.pi) for w in CoFeBchar.GetSecondPerturbation(n=0, nc=3)]

plt.plot(kxi*1e-6, w00, kxi*1e-6, w22, kxi*1e-6, wd00, kxi*1e-6, wd22);
plt.xlabel('kxi (rad/um)');
plt.ylabel('Frequency (GHz)');
plt.legend(['00', '33', '03', '30'])
...
plt.xlabel('kxi (rad/um)');
plt.ylabel('Frequency (GHz)');
plt.legend(['TP 00', 'TP 11', 'dp = 1e11 00', 'dp = 1e11 11', ])
