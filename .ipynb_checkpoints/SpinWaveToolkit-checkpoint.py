...
class DispersionCharacteristic:
    ...
    propLen = NiFeChar.GetPropLen()*1e6 #um \n
    """
    def __init__(self, Bext, material, d, kxi = np.linspace(1e-12, 25e6, 200), theta = np.pi/2, phi = np.pi/2, weff = 3e-6, boundaryCond = 1, dp=0, Ku = 0, Ku2 = 0,  Jbl=0, Jbq=0, s=0, d2=0, material2=0, JblDyn=1, JbqDyn=1, phiAnis1=np.pi/2, phiAnis2=np.pi/2, phiInit1=np.pi/2, phiInit2=-np.pi/2):
