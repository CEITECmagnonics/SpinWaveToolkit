...
def getBLSsignal(SweepBloch, KxKyBloch, Bloch, Exy, E, DF, PM, d, NA, Nq=30,
                 source_layer_index=1, output_layer_index=0, 
                 wavelength=532e-9, collectionSpot=1e-6, focalLength=1e-3):
    """
    ...
    for comp in range(3):
        ...
        interp_func = RegularGridInterpolator((kx_fft, ky_fft),
                                                fftEI[comp, :, :],
    ...
    ExtinCoefMagLayer = np.sqrt((abs(DF[source_layer_index])-np.real(DF[source_layer_index]))/2)
    ...
    htp, hts = fresnel_coefficients(lambda_=wavelength, DF=DF, PM=PM,
                                        d=d, source_layer_index=source_layer_index, output_layer_index=output_layer_index)
    ...
    pGF, sGF = sph_green_function(Kx=Qx, Ky=Qy, DFMagLayer=DF[source_layer_index], wavelength=wavelength, tp=tp_fixed, ts=ts_fixed)
    # -------------------------------------------------------------
    ...
    for i, s in enumerate(SweepBloch):
        ...
        interp_Mx = RegularGridInterpolator((kx_grid, ky_grid),
                                              Bloch[0, i, :, :],
                                              bounds_error=False, fill_value=0)
        interp_My = RegularGridInterpolator((kx_grid, ky_grid),
                                              Bloch[1, i, :, :],
                                              bounds_error=False, fill_value=0)
        interp_Mz = RegularGridInterpolator((kx_grid, ky_grid),
                                              Bloch[2, i, :, :],
                                              bounds_error=False, fill_value=0)
        ...
        Px = (convolve2d(interp_fftEI[2, :, :], 1j * Bloch_interp_My, mode='same') +
              convolve2d(interp_fftEI[1, :, :], -1j * Bloch_interp_Mz, mode='same'))
        Py = (convolve2d(interp_fftEI[0, :, :], 1j * Bloch_interp_Mz, mode='same') +
              convolve2d(interp_fftEI[2, :, :], -1j * Bloch_interp_Mx, mode='same'))
        Pz = (convolve2d(interp_fftEI[1, :, :], 1j * Bloch_interp_Mx, mode='same') +
        ...
        Ep = (pGF[0][0] * Px * np.exp(-1j * Kzs * d[source_layer_index-1]) +
              pGF[0][1] * Px * np.exp(1j * Kzs * d[source_layer_index-1]))
        Ep += (pGF[1][0] * Py * np.exp(-1j * Kzs * d[source_layer_index-1]) +
               pGF[1][1] * Py * np.exp(1j * Kzs * d[source_layer_index-1]))
        Ep += (pGF[2][0] * Pz * np.exp(-1j * Kzs * d[source_layer_index-1]) +
               pGF[2][1] * Pz * np.exp(1j * Kzs * d[source_layer_index-1]))

        Es = (sGF[0][0] * Px * np.exp(-1j * Kzs * d[source_layer_index-1]) +
              sGF[0][1] * Px * np.exp(1j * Kzs * d[source_layer_index-1]))
        Es += (sGF[1][0] * Py * np.exp(-1j * Kzs * d[source_layer_index-1]) +
               sGF[1][1] * Py * np.exp(1j * Kzs * d[source_layer_index-1]))
        Es += (sGF[2][0] * Pz * np.exp(-1j * Kzs * d[source_layer_index-1]) +
               sGF[2][1] * Pz * np.exp(1j * Kzs * d[source_layer_index-1]))
        # -------------------------------------------------------------
        ...
        Factor = (-2j * np.pi * np.sqrt(Kz * k0)) * np.exp(1j * k0 * focalLength) / focalLength
        Ex_field *= Factor
        Ey_field *= Factor
        # -------------------------------------------------------------
        ...
        factor_fft = (focalLength / k0) ** 2 * Ex_field.size / (4 * np.pi**2) * dkx * dky
